...
Built for the Jeremy paid-test: one client brain, claims allowlist, confirm-before-spend, per-client hard cap. Video backends are **stubs**. This repo will not rent a GPU or call MiniMax paygo.
...
| --- | --- | --- |
| **RunPod self-host MiniMax H3** | **~$0.17** | Unverified hypothesis from the call. Default stub. **No pods spun.** |
...
7. `assemble` — ffmpeg concat + `handoff.json` / `EDITOR.md`
8. `cost` — per-client ledger
...
| `MINIMAX_API_KEY` / `RUNPOD_API_KEY` | Reserved. Unused. Do not paste keys into chat; HQ secrets later |
...
- `runpod-h3` — self-host H3 placeholder at $0.17 / 5s
...
Web UI. Live GPU rental. Real MiniMax / RunPod spend. Media-buying automation. Multi-client HQ production rollout. Self-host H3 production install.
